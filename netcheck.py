...
#######################################################
def logOldIp(oldIp):
    logFile = open(IP_LOG_FILE, 'a')
    logFile.write(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ' - Old IP: '+oldIp +'\n')
    logFile.close()
...
def hasIpChanged(currentIp):
    ...
    lastFile = open(LAST_IP_FILE, 'r+')
    lastIp = lastFile.read()
    if (lastIp != currentIp):
        ipChanged = True
        logOldIp(lastIp)
        lastFile.seek(0)
        lastFile.write(currentIp)
        lastFile.truncate()
    lastFile.close()
    return ipChanged
